...
const addSaleEntryToArray = (qty, rate) => {
  const saleEntry = {
...
const displaySaleEntries = () => {
  ...
  if(element) {
    ...
    saleEntries.forEach(e => {
      ...
      element.innerHTML += `
        <tr id='entry-${saleEntries.indexOf(e)}'>
          <td>${++i}</td>
          <td>${e.itemName}</td>
          ...
          <td>
            <a class='saleEntry' id='saleEntry-${saleEntries.indexOf(e)}' href='#'>
...
function removeSaleEntry(rowId) {
  if(saleEntries.length > rowId) {
    saleEntries.splice(rowId, 1);
    displaySaleEntries();
  }
}
  
const createSaleEntryForm = document.querySelector('.form-body-child').addEventListener('submit', e => {
  e.preventDefault();
  ...
  //  // Use below code to ensure stock availability
  // if(qty > availableQty) {
  ...
  addSaleEntryToArray(parseFloat(qty), parseFloat(rate));
  resetQtyBlock();
});
    ...
    if(itemId) {
      try {
        ...
        if(res.data.status === 'success') {        
          // console.log(document.getElementById('qtyBlock').innerHTML);
...
const createSale = async () => {
  try {
    ...
    if(res.data.status === 'success') {
      window.setTimeout(() => {
        printInvoice('sales', res.data.data, saleEntries);
...
const createSaleForm = document.querySelector('.form-body').addEventListener('submit', e => {
  e.preventDefault();
